...
class ExperimentCellVitPanNuke(BaseExperiment):
    ...
    def run_experiment(self) -> tuple[Path, dict, nn.Module, dict]:
        ...
        ###########################  customized class-based sampler ##############################################
        if isinstance(train_dataset, Subset):
            ...
            csv_paths = []
            for csv_path in self.run_conf['data']['train_folds']:
                csv_paths.append(os.path.join(self.run_conf['data']['dataset_path'], f"fold{csv_path}", 'types.csv'))
            ...
            references = pd.concat(dataframes, ignore_index=True)
    ...
    ###################################################################################################
        ...
        val_dataloader = DataLoader(
            val_dataset,
            batch_size=16, # it was 128
            num_workers=8,#default 16
            pin_memory=True,
            worker_init_fn=self.seed_worker,
            drop_last=True, ##fix: add. current: need to do: trainer_cellvit --> validation_epoch, there is extra dimension when batch size is 1
            shuffle=True,
        )
        ...
        trainer.fit(
            ...
            unfreeze_epoch=self.run_conf["training"]["unfreeze_epoch"],  #unfreeze epoch number
    ...
    def get_loss_fn(self, loss_fn_settings: dict) -> dict:
        ...
        ## fix : disable default NT branch for instance segmentation only
        ...
        ## fix: disable default Tissue classification for unknown tissue
    ...
    def get_datasets(
        ...
        ##fix: change Dataset class config to 'pannuke_like_instance'

        full_dataset = select_dataset(
            # dataset_name="pannuke", # default
            dataset_name="pannuke_like_instance",
            split="train",
            dataset_config=self.run_conf["data"],
            transforms=train_transforms,
        )
        if "val_split" in self.run_conf["data"]:
            ######### add by me: cross validation
            ...
            ####### add by me: specify the folds for validation directly.
        ...
        **kwargs,
        ...
        if backbone_type.lower() == "vit256":
            ...
            if pretrained_model is not None:
                ...
                # self.logger.info(model.load_state_dict(cellvit_pretrained, strict=True)) # default
                self.logger.info(model.load_state_dict(cellvit_pretrained['model_state_dict'], strict=True)) ##fix

                # ##fix: add for loading pretrained whole model [encoder + decoder]
                # model.unfreeze_encoder()
                # return model
            model.freeze_encoder() # default
    ...
    def get_transforms(
        ...
        ##fix: add, crop the image into 256x256 shape
        transform_list.append(A.RandomCrop(width=input_shape, height=input_shape, p=1))
        ...
        ##fix: add resize to input shape (default is: A.Normalize() only
        val_transforms = A.Compose([A.RandomCrop(height=input_shape, width=input_shape, p=1),
